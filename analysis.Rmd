...
relationen_vorstand_adv <- merge(x = firmen, 
                                   y = relationen_vorstand, 
                                   by = "firm_id", 
                                   all.x = FALSE, 
                                   all.y = TRUE)
...
```{r}
diversity_branche <- function(df, branchen_code) {
  df_counts <- df %>%
    filter(branchen_code == branchen_code) %>%
  group_by(beginn_jahr, geschlecht) %>%
  summarize(count = n())
  ...
  ggplot(df_freq, aes(x = beginn_jahr, y = freq, color = geschlecht)) +
    ...
    scale_y_continuous(labels = scales::percent) +
    labs(x = "Year", y = "Relative Frequency", color = "Gender")
  ggsave(paste("Plots/diversity_", branchen_code, ".png", sep = ""),
         width = 12, height = 9)
}
```
...
for (branchen_code in branchen_codes) {
  diversity_branche(relationen_vorstand_adv, branchen_code)
}
```
