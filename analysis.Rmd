...
relationen_fuehrung_adv_stat <- relationen_fuehrung_adv_stat %>%
  group_by(firm_id) %>%
  mutate(diversity_score = mean(min(c(max(alter) - min(alter)) / 50, 1), mean(geschlecht == "m")))


relationen_fuehrung_adv_stat %>% arrange(desc(diversity_score)) %>% select(firmenname, hr_nummer, diversity_score, ma_range)
```
