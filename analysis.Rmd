...
relationen_fuehrung_adv_stat <- relationen_fuehrung_adv_stat %>%
  ...
  mutate(diversity_alter = min(c(max(alter) - min(alter)) / 50, 1),
         ...
         diversity_score = mean(diversity_alter[1], diversity_geschlecht[1]))
...
relationen_fuehrung_adv_stat %>% arrange(desc(diversity_score)) %>% select(firmenname, hr_nummer, diversity_score, mitarbeiter_range) %>% filter(mitarbeiter_range %in% c("501-5000", "5001-50000")) %>% distinct(firmenname, hr_nummer, diversity_score, mitarbeiter_range)
```
